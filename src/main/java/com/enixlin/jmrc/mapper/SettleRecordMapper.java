...
public interface SettleRecordMapper extends BaseMapper<SettleRecord> {
	...
	@Select("SELECT sum(usd_Rate * busy_Amount) as performance ,product_Name as name from settle_record where busy_Date>='#{param0}' and busy_Date<='#{param1}' and product_Name in (#{param2}) GROUP BY name ")
