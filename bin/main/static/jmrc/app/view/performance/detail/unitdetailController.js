Ext.define('jmrc.view.performance.detail.unitdetailController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.performance-detail-unitdetail',
    afterRender: function() {
        let me = this;
        let view = me.getView();
        let startDay = me.getView()["config"]["data"]["start"];
        let endDay = me.getView()["config"]["data"]["end"];
        let st = "allUnitPerformanceStore";
        let bindStore = me.getViewModel().getStore(st);
        
        view.width=window.innerWidth*0.8;
        let grid = Ext.create("Ext.grid.Panel", {
            width: window.innerWidth * 0.75,
            height: window.innerHeight * 0.65,
            border: 2,
            scrollable: true,
            tbar:["->",{text:"导出表格：",handler:"exportExcel"}],
        });
