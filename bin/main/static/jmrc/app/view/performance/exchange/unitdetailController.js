Ext.define('jmrc.view.performance.exchange.unitdetailController', {
    ...
    afterRender: function() {
        ...
        let grid = Ext.create({
            ...
            border: 2,
            store: {
                fields: ["branchId", "branchName", "amount", "times", "times_compare", "amount_compare", ],
                proxy: {
            ...
            columns: [
                ...
                { header: "<center>行名</center>", dataIndex: "branchName",
                	renderer:function(v){
                		return v.replace("本部","").replace("（一级支行）","");
                	}
                
                },
                { header: "<center>笔数</center>", dataIndex: "times",
                	renderer:function(v){
                		return Ext.util.Format.number(v,'0,000');
                	}
                
                },
                { header: "<center>金额<br>（万美元）</center>", dataIndex: "amount" ,
                	renderer:function(v){
                		return Ext.util.Format.number(v,'0,000.00');
                	}	
                },
                { header: "<center>笔数<br>（同比）</center>", dataIndex: "times_compare",
                	renderer:function(v){
                		if(v>0){
                ...
                { header: "<center>金额<br>（万美元）</center>", dataIndex: "amount_compare",
                	renderer:function(v){
                		if(v>0){
